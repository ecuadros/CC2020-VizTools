...
@RestController
@RequestMapping("/v1/dka")
@RequiredArgsConstructor
public class DKAController {

	private final DKAService service;

	@PreAuthorize("hasRole('ROLE_ADMIN')")
	@PostMapping("/")
	public ResponseEntity<?> create(@RequestBody DKADto input) {
	...
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	@PutMapping("/{id}")
	public ResponseEntity<?> update(@PathVariable Long id, @RequestBody DKADto input) {
